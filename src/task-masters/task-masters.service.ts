...
export class TaskMastersService {
  ...
  async findAll(filters: { isActive?: boolean; categoryId?: number }) {
  ...
  async assignClients(taskMasterId: number, dto: AssignClientsDto) {
    ...
    const ops = dto.clientIds.map((clientId) =>
      this.prisma.taskMasterClient.upsert({
        ...
        update: {
          ...(dto.customDueDay !== undefined
            ? { customDueDay: dto.customDueDay }
            : {}),
          ...(dto.startDate !== undefined
            ? { startDate: dto.startDate ? new Date(dto.startDate) : null }
            : {}),
  ...
  private async generateEventBasedTasks(
    ...
    const periodStart = new Date(master.startDate);
    const periodEnd = master.endDate ? new Date(master.endDate) : null;

    for (const link of master.clients) {
      if (!link.isActive) continue;

      const didCreate = await this.createRecurringTaskIfNotExists({
        master,
  ...
  private parseFinancialYear(financialYear: string) {
    ...
    if (!fyStartYear || fyStartYear < 2000 || fyStartYear > 2100) {
      throw new BadRequestException(
        'Invalid financialYear format (expected 2025-26)',
      );
    }
  ...
  private quarterRangeUTC(fyStartYear: number, quarter: number) {
    ...
    if (q === 1)
      return {
        ...
        end: new Date(Date.UTC(fyStartYear, 6, 0, 23, 59, 59)),
      };
    if (q === 2)
      return {
        start: new Date(Date.UTC(fyStartYear, 6, 1)),
        end: new Date(Date.UTC(fyStartYear, 9, 0, 23, 59, 59)),
      };
    if (q === 3)
      return {
        start: new Date(Date.UTC(fyStartYear, 9, 1)),
        end: new Date(Date.UTC(fyStartYear, 12, 0, 23, 59, 59)),
      };
    // q === 4 (Jan-Mar of next calendar year)
    return {
      start: new Date(Date.UTC(fyStartYear + 1, 0, 1)),
      end: new Date(Date.UTC(fyStartYear + 1, 3, 0, 23, 59, 59)),
    };
  }
  ...
  private async createRecurringTaskIfNotExists(params: {
    master: any;
    clientId: number;
    periodStart: Date;
    periodEnd: Date | null;
    ...
    try {
      await this.prisma.$transaction(async (tx) => {
        const task = await tx.task.create({
          data: {
            title: params.title,
            description: params.master.description ?? null,
            clientId: params.clientId,
            status: 'PENDING',
            taskMasterId: params.master.id,
            categoryId: params.master.categoryId,
            isBillable: params.finalIsBillable,
            hsnSac: params.finalIsBillable ? params.finalHsnSac : null,
            gstRate: params.finalIsBillable ? params.finalGstRate : null,
            unitLabel: params.finalIsBillable ? params.finalUnitLabel : null,
            periodStart: params.periodStart,
        ...
        if (params.assignedToUserId) {
          await tx.taskAssignment.create({
            data: { taskId: task.id, userId: params.assignedToUserId },
          });
        }
      });
  ...
  private async generateDailyTasks(
    ...
    if (!dto.financialYear) {
      throw new BadRequestException(
        'financialYear is required for DAILY generation',
      );
    }
    ...
    let rangeEnd = fy.end;
    let periodLabel = dto.financialYear;
    ...
    if (dto.month) {
      ...
      periodLabel = `${dto.financialYear}-${String(m).padStart(2, '0')}`;
    }
    ...
    for (const link of activeClientLinks) {
      ...
      const clamped = this.clampToMasterAndLinkRange(
        masterStart,
        masterEnd,
        linkStart,
        linkEnd,
      ...
      const didCreate = await this.createRecurringTaskIfNotExists({
        ...
        finalGstRate,
        finalUnitLabel,
        title: `${master.title} - ${periodLabel}`,
      });
    ...
    return {
      ...
      period: { periodStart: rangeStart, periodEnd: rangeEnd },
  ...
  private async generateWeeklyTasks(
    ...
    if (!dto.financialYear) {
      throw new BadRequestException(
        'financialYear is required for WEEKLY generation',
      );
    }
  ...
  private async generateMonthlyTasks(
    ...
    const { start: periodStart, end: periodEnd } = this.monthRangeUTC(
      year,
      month,
    );
  ...
  private async generateQuarterlyTasks(
    ...
    if (!dto.financialYear)
      throw new BadRequestException('financialYear is required for QUARTERLY');
    if (!dto.quarter)
      throw new BadRequestException('quarter is required for QUARTERLY (1..4)');

    const fy = this.getFyRange(dto.financialYear);
    const { start: periodStart, end: periodEnd } = this.quarterRangeUTC(
      fy.fyStartYear,
      dto.quarter,
    );
    const periodLabel = `${dto.financialYear}-Q${dto.quarter}`;
  ...
  private async generateYearlyTasks(
    ...
    if (!dto.financialYear)
      throw new BadRequestException('financialYear is required for YEARLY');

    const fy = this.getFyRange(dto.financialYear);
    const periodStart = fy.start;
    const periodEnd = fy.end;
    const periodLabel = dto.financialYear;
  ...
  async generateTasksForPeriod(taskMasterId: number, dto: GenerateTasksDto) {
    ...
    if (!master) throw new NotFoundException('TaskMaster not found');
    if (!master.isActive)
      throw new BadRequestException('TaskMaster is not active');
    ...
    const finalIsBillable =
      dto.isBillable !== undefined ? dto.isBillable : master.isBillable;

    const finalHsnSac = dto.hsnSac !== undefined ? dto.hsnSac : master.hsnSac;

    const finalGstRateRaw =
      dto.gstRate !== undefined ? dto.gstRate : master.gstRate;
    ...
    const finalUnitLabel =
      dto.unitLabel !== undefined ? dto.unitLabel : master.unitLabel;
  ...
  async getGeneratedTasks(
    ...
    return this.prisma.task.findMany({
      ...
      include: {
        client: {
          select: { id: true, name: true, code: true, gstNumber: true },
        },
      ...
      try {
        ...
        await this.prisma.taskMaster.create({
          data: {
            ...
            gstRate: row['GST Rate'] ? Number(row['GST Rate']) : null,
